...
QueryResult *SQLExec::insert(const InsertStatement *statement) {
    ...
    for (uint i = 0; i < statement->values->size(); i++) {
        ...
        if (statement->columns != NULL)
            column = statement->columns->at(i);
        else 
            column = trueNames.at(i);
...
QueryResult *SQLExec::del(const DeleteStatement *statement) {
    ...
    ValueDict* where = nullptr;
    if (statement->expr != nullptr)
        where = get_where_conjunction(expr);
    // make eval plan
    ...
    if (where) {
        EvalPlan selectPlan = EvalPlan(where, plan);
...
QueryResult *SQLExec::drop_index(const DropStatement *statement) {
    ...
    //return query
    return new QueryResult("dropped index " + indexID);
